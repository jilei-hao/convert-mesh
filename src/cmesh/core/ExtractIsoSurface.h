...
struct IsoSurfaceParams
{
  ...
  double    smooth_pre      = 0.0;   // Gaussian std-dev in voxels; continuous methods only
...
/**
 * Check an IsoSurfaceParams for option combinations that do not apply to
 * the chosen method, throwing AlgorithmError. In particular `smooth_pre`
 * is rejected for discrete methods: Gaussian-smoothing a label map blends
 * adjacent label values into spurious intermediate labels. Smooth the
 * output mesh instead (SmoothMesh / -smooth-mesh).
 *
 * ExtractIsoSurface calls this itself; it is public so hosts (CLI,
 * bindings) can validate early and attribute the error to user input.
 */
